...
export function Hero() {
  ...
  return (
    ...
    >
      ...
      <div
        className="absolute bottom-0 left-1/4 w-40 h-40 rounded-full bg-saffron/10 blur-3xl animate-smoke"
        style={{ animationDelay: "0s" }}
      />
      <div
        className="absolute bottom-0 right-1/3 w-52 h-52 rounded-full bg-gold/10 blur-3xl animate-smoke"
      ...
      <div className="relative z-10 max-w-4xl mx-auto px-6 text-center pt-12 pb-16">
        ...
        >
          ...
          <span className="text-xs uppercase tracking-[0.25em] text-cream/80">{hoursStatus}</span>
        </motion.div>
        ...
        >
          ...
          <p className="text-xs sm:text-sm uppercase tracking-[0.4em] text-gold mb-4">
        ...
        >
          <a
            href="#menu"
            className="group inline-flex items-center gap-2 px-7 py-3 rounded-full bg-gradient-gold text-primary-foreground font-medium tracking-wide shadow-gold hover:scale-105 transition-transform animate-glow"
          >
            <UtensilsCrossed size={18} /> View Menu
          </a>
          <a
            href={RESTAURANT.phoneLink}
            className="inline-flex items-center gap-2 px-7 py-3 rounded-full border border-gold/60 text-gold hover:bg-gold/10 transition"
          >
        ...
        >
          <div className="flex items-center justify-center gap-2">
            <MapPin size={14} className="text-gold" /> 8 Gordon Street, Luton LU1 2QP
          </div>
