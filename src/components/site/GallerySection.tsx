...
const items = [
  { src: shopfront, label: "Shop Front", span: "sm:col-span-2 sm:row-span-2" },
  { src: curries, label: "Curries" },
  { src: biryani, label: "Biryanis" },
  { src: starters, label: "Starters", span: "sm:col-span-2" },
];

export function GallerySection() {
  return (
    <section id="gallery" className="py-24 sm:py-32 px-6">
      <div className="max-w-6xl mx-auto">
        <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="text-center mb-14">
          <p className="text-xs uppercase tracking-[0.4em] text-gold mb-4">Gallery</p>
          <h2 className="font-serif text-4xl sm:text-6xl text-gradient-gold mb-4">A Glimpse Inside</h2>
          <div className="gold-divider w-40 mx-auto" />
        </motion.div>

        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 auto-rows-[180px] sm:auto-rows-[220px]">
          {items.map((it, i) => (
            <motion.div
              key={it.label}
              initial={{ opacity: 0, scale: 0.95 }} whileInView={{ opacity: 1, scale: 1 }}
              viewport={{ once: true }} transition={{ duration: 0.6, delay: i * 0.08 }}
              className={`relative ornate-border overflow-hidden group cursor-pointer ${it.span ?? ""}`}
            >
              <img src={it.src} alt={it.label} loading="lazy" width={1024} height={1024}
              ...
              <div className="absolute bottom-4 left-4 right-4">
                ...
                <h3 className="font-serif text-2xl text-cream">{it.label}</h3>
              </div>
            </motion.div>
          ))}
        </div>
