        ...
        <nav className="hidden md:flex items-center gap-8">
          {links.map((l) => (
            <a key={l.href} href={l.href}
              className="text-sm uppercase tracking-[0.2em] text-cream/80 hover:text-gold transition-colors relative group">
              {l.label}
              <span className="absolute -bottom-1 left-0 w-0 h-px bg-gold transition-all group-hover:w-full" />
            </a>
          ))}
        </nav>
        <button className="md:hidden text-gold z-50" onClick={() => setOpen((o) => !o)} aria-label="Menu">
